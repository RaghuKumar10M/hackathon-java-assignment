...
class WarehouseConcurrencyIT {
   ...
   */
  @Test
  void testConcurrentWarehouseCreationWithUniqueCodesSucceeds() throws InterruptedException {
    int threadCount = 10;
    ...
    for (int i = 0; i < threadCount; i++) {
      ...
      Future<Boolean> future = executor.submit(() -> {
        try {
          ...
          warehouse.setBusinessUnitCode("CONCURRENT-" + index);
   ...
   */
  @Test
  void testConcurrentWarehouseCreationWithDuplicateCodeFails() throws InterruptedException {
    int threadCount = 5;
    ExecutorService executor = Executors.newFixedThreadPool(threadCount);
    CountDownLatch latch = new CountDownLatch(threadCount);
    ...
    for (int i = 0; i < threadCount; i++) {
      executor.submit(() -> {
        try {
          Warehouse warehouse = new Warehouse();
          warehouse.setBusinessUnitCode(duplicateCode);  // Same code for all!
   ...
   */
  @Test
  void testConcurrentReadsAreNonBlocking() throws InterruptedException {
    // Create a warehouse first (in its own committed transaction)
    String testCode = "READ-TEST-" + System.currentTimeMillis();
    Warehouse warehouse = new Warehouse();
    warehouse.setBusinessUnitCode(testCode);
    warehouse.setLocation("AMSTERDAM-001");
    ...
    for (int i = 0; i < readThreadCount; i++) {
      executor.submit(() -> {
        try {
          Warehouse found = warehouseRepository.findByBusinessUnitCode(testCode);
          if (found != null) {
